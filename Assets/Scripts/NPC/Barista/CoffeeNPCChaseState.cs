...
public class CoffeeNPCChaseState : CoffeeNPCStateBase
{
    ...
    public override void OnEnter()
    {
        ...
        if (value > 0.5f)
        {
            if (value <= 0.7f)
            {
                DialogManager.Instance.ShowDialog("哪来的臭鸟", Npc.transform.position, new Vector3(0, 2.5f, 0), Npc.transform);
            }
            else if (value <= 0.9f)
            {
                DialogManager.Instance.ShowDialog("你完蛋了", Npc.transform.position, new Vector3(0, 2.5f, 0), Npc.transform);
            }
            else
            {
                DialogManager.Instance.ShowDialog("嗨嗨  来了嗷", Npc.transform.position, new Vector3(0, 2.5f, 0), Npc.transform);
            }
        }
